...
> **Status: COMPLETE on `develop` (bar one roadmap item).** Approved 2026-07-01; fully implemented by 2026-07-02. **All of V2-B1…B11 are landed** — the Critical + every High, the full B9 SOLID/DEBT refactor set (on the HTTP integration harness), and every B11 enforcement + CI-infra gate — test-first, merged to `develop`; build clean (warnings-as-error), Core 42/42, Api 407/407 green, CI green (build-test + secret-scan + license-scan + qa-artifacts). **One scoped item remains: B8-5** — the full Playwright **E2E-in-CI** journeys behind a bootable app+DB stack (the roadmap's pre-existing E2E debt, not a v2-audit correctness/enforcement gap). This plan consolidates Phases 1–4 (all pinned to `84c7ad8`). Order mirrors the archived `audits/v1-2026-06/AUDIT_TASKS.md`: **keystone first, enforcement last.** Each task lists severity, the rule/finding it satisfies, whether it **touches core**, the **tests that must exist first** (TDD), a **done-when**, and a **verify** command.
>
...
| **V2-B8** | Test-completeness (test-first) | B8-1…B8-6 | Critical (tests) | tests only | ✅ Done (cross-tenant negatives, HTTP harness, MFA step-up on all 4 paths) — only **B8-5 E2E-in-CI** remains¹ |
...
¹ **Remaining item (one):**
- **B8-5 — E2E-in-CI.** Specify + wire the missing Playwright journeys (OAuth, magic-link, tenant-isolation, RBAC three-tier, MFA enroll+step-up, GDPR export/erasure, admin impersonation, i18n) behind a **bootable full stack** (Postgres + API + WASM) in CI; billing E2E additionally needs a fake-provider E2E seam. This is the roadmap's pre-existing E2E debt — a standalone infrastructure effort, not a v2-audit correctness/enforcement gap. The service + **HTTP-integration** layers it would sit above are already covered (the `IntegrationTestFactory` harness + the MFA step-up integration tests exercise the real app end-to-end, sans browser).
>
> *All other scoped follow-ups from earlier drafts are now DONE:* the HTTP harness (B8-6), MFA step-up on every path (B8-2), the full B9 SOLID/DEBT set (B9-1/2/4/5/6/7), and every B11 gate incl. CI-infra (secret-scan, CPM+lockfile, license-scan, QA-artifacts, arch gates R6/R13/R24/R35, MailKit ban).
