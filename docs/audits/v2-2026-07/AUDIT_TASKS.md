...
- [~] **B9-6 · Move PUBAPI/HOOKS per D7; extract `TenantDissolutionService`; branded notification email** — Medium · (DEBT-6/7/8, TR-7) — ✅ **DEBT-6 done**: PUBAPI/HOOKS (`ApiKeyEndpoints`/`WebhookEndpoints`) + the shared endpoint-extension helpers moved `Features/` → `src/Api/Endpoints/` (`Template.Api.Endpoints`); `Features/` now holds only the Notes sample; ADR-004 amended (D7); the **R6 gate** (`FeatureFiles_...NotRawMapGroup`) landed → **B11-1 complete**. ⬜ **DEBT-7** (`TenantDissolutionService`) + **DEBT-8** (branded notification email) folded into B9-7 (PR9).
