...
```powershell
dotnet publish src/Maui/Perezosoft.Maui.csproj -f net10.0-android -c Release `
...
```powershell
dotnet publish src/Maui/Perezosoft.Maui.csproj -f net10.0-windows10.0.19041.0 -c Release `
  -p:ApiBaseUrl=https://<your-host> -p:WindowsPackageType=None -o out/windows
```

Run `out/windows/Perezosoft.Maui.exe` from that folder (pin a shortcut). SmartScreen warns once because it is
