...
auth actually validates with." (This is the platform's blessed shape per rule R22; .NET's
`IOptions` + `ValidateOnStart` is a fine alternative — the *decision* is eager validation
and one pattern everywhere, not which library performs it.)
...
the failure message. The documentation *cannot* drift from the code, because drift fails
