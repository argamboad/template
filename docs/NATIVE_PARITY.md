...
| **Auth: OTP / OAuth / MFA step-up** | ✅ | All native-wired and previously verified: OTP + OAuth via system browser (`LoopbackOAuthInitiator` desktop, custom scheme Android), MFA-4 native step-up, provider discovery (`GET /api/auth/providers`) works over the native client (anonymous endpoint). Magic link N-A by design. **2026-07-07 hardening:** the Android manifest was missing the browser `<queries>` (package visibility), so WebAuthenticator fell back from a Custom Tab to full external Chrome — on Android 16 the backgrounded app got killed mid-OAuth and the `perezosoft://` callback cold-started into a process with no pending auth (app flashes open, closes). `<queries>` for `VIEW https` + `CustomTabsService` added; Custom Tabs keep the app task foreground for the round trip. |
