...
| `docs/stories/billing.md` | epic `BILLING` ✅ COMPLETE — entitlements + Checkout + webhook + Portal (1–4) + seat/usage quotas (5, `IQuotaService`) + trial/dunning (6, `IBillingNotifier` + lapse sweep via NOTIFY) + dissolve cleanup (7, `BillingDataContributor` cancels the provider sub + wipes the projection); ADR-006 |
...
| `docs/stories/e2e.md` | epic `E2E` 📝 planned — Playwright journeys for platform UI built unit/integration-first: RBAC roster (E2E-1), billing seat-quota 402 UX (E2E-2), notification bell/prefs (E2E-3); starts after the v2 audit remediation lands |
| `docs/stories/deploy.md` | epic `DEPLOY` 🚧 — staging/prod on the free tier: DEPLOY-1 ✅ single-origin (API serves the WASM) + config-gated forwarded headers; DEPLOY-2 🚧 Dockerfile + compose parity + `render.yaml` + `docs/DEPLOYMENT.md` (local half done, cloud bring-up = operator); DEPLOY-3 📝 deploy pipeline (develop→staging auto + smoke, main→prod gated) + QA staging section; ADR-017 |
| `docs/DEPLOYMENT.md` | Deployment runbook (DEPLOY-2) — Render + Neon + Brevo free-tier bring-up; `Dockerfile` + `render.yaml` reference; required env incl. the Production Stripe-key guard |
