...
- **Versions:** latest stable on the current .NET line — **.NET SDK 10.0.303 (pinned in `global.json`, the single source of truth), ASP.NET Core / EF Core packages 10.0.9, Npgsql.EF 10.0.2, PostgreSQL 17.** The SDK is **pinned, not floating** (v3 audit DEP-4): CI's `setup-dotnet` reads `global-json-file: global.json`, and both Dockerfile image tags (`sdk:10.0.303` build, `aspnet:10.0.9` runtime) match it — so a runner-image SDK patch can't outrun the committed `packages.lock.json` (the WASM SDK injects patch-sensitive implicit packages → NU1004 in locked-mode restore).
