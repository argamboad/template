...
| `docs/stories/native.md` | epic `NATIVE` 🚧 — full MAUI parity (Android/Windows/iOS/macOS): NATIVE-1 ✅ CI build gate (all 4 TFMs; Apple legs on develop pushes — 10× macOS minutes; Maui lockfile excluded by design) + NATIVE-2 ✅ parity audit → gaps G1–G6; next NATIVE-3 (downloads) / 4 (external nav + back button) / 4b (join-by-code) / 5 (culture); NATIVE-6 QA pass + NATIVE-7 emulator smoke; NATIVE-8/9/10 signed AAB/MSIX/IPA/pkg + NATIVE-11 store submission; ADR-018 |
