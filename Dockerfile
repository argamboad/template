...
COPY src/Core/Template.Core.csproj src/Core/packages.lock.json src/Core/
COPY src/Infrastructure/Template.Infrastructure.csproj src/Infrastructure/packages.lock.json src/Infrastructure/
COPY src/Api/Template.Api.csproj src/Api/packages.lock.json src/Api/
COPY src/Shared.Ui/Template.Shared.Ui.csproj src/Shared.Ui/packages.lock.json src/Shared.Ui/
COPY src/Web/Template.Web.csproj src/Web/packages.lock.json src/Web/
...
RUN dotnet restore src/Api/Template.Api.csproj --locked-mode \
 && dotnet restore src/Web/Template.Web.csproj
...
RUN dotnet publish src/Web/Template.Web.csproj -c Release --no-restore -o /publish/web \
 && dotnet publish src/Api/Template.Api.csproj -c Release --no-restore -o /publish/api \
...
ENTRYPOINT ["sh", "-c", "exec dotnet Template.Api.dll --urls http://0.0.0.0:${PORT:-8080}"]
